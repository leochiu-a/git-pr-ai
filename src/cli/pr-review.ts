...
async function parsePRUrl(prUrl: string): Promise<PRInfo> {
  ...
  try {
    const result = await $`gh pr view ${prNumber} --repo ${owner}/${repo} --json baseRefName,headRefName,url`;
  ...
  try {
    ...
    if (prUrl) {
      ...
      if (!prInfo) {
        ...
        console.error("Please provide a PR URL or switch to a branch with an existing PR");
