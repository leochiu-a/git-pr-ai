...
- f6a383e: fix(open-pr): correctly detect fork workflow when both origin and upstream remotes exist
...
- 694f0da: Standardize non-interactive CLI behavior across PR and commit workflows.
...
- c0efeb0: Fix `open-pr --no-web` on GitHub by providing an explicit empty PR body in non-interactive mode.
...
- 7dbdf8b: Improve the `git pr-ai config` flow with agent-specific model selection:
